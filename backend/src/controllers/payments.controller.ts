...
export const initiateESewa = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const paymentData = await esewaService.initiateESewa(parseFloat(order.total as string), order.id);
...
export const verifyESewa = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { data } = req.body;
