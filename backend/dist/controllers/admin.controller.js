...
const normalizeSlug = (value) => value
    ...
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/(^-|-$)/g, '');
const isProductType = (value) => typeof value === 'string' && Object.values(ProductType).includes(value);
...
const buildProductResponse = (product) => ({
    ...
    primaryImageUrl: (product.images ?? []).find(image => image.isPrimary)?.url ??
...
const mapProductImages = (images, productName) => {
...
const mapProductVariants = (variants) => (variants ?? [])
    .map(variant => ({
...
const mapDesignAreas = (designAreas) => (designAreas ?? [])
    .map((area, index) => ({
...
export const getDashboardStats = async (req, res, next) => {
    try {
        ...
        const monthlyRevenueRaw = await AppDataSource.query(`
        SELECT
...
export const createProduct = async (req, res, next) => {
    try {
        const { name, description, slug, productType, basePrice, material, fabric, gsm, weightGrams, weight, printTechnique, metaTitle, metaDescription, tags, isActive, isCustomizable, isFeatured, categoryId, images, variants, designAreas, } = req.body;
...
export const updateProduct = async (req, res, next) => {
    try {
        ...
        if (typeof description !== 'undefined')
            product.description = String(description ?? '').trim() || null;
        ...
        if (typeof basePrice !== 'undefined')
            product.basePrice = toNumber(basePrice, 0);
        ...
        if (typeof gsm !== 'undefined')
            product.gsm = toOptionalNumber(gsm);
        if (typeof weightGrams !== 'undefined' || typeof weight !== 'undefined') {
            product.weightGrams = toOptionalNumber(weightGrams ?? weight);
        }
        if (typeof printTechnique !== 'undefined')
            product.printTechnique = String(printTechnique ?? '').trim() || null;
        if (typeof metaTitle !== 'undefined')
            product.metaTitle = String(metaTitle ?? '').trim() || null;
        if (typeof metaDescription !== 'undefined')
            product.metaDescription = String(metaDescription ?? '').trim() || null;
        ...
        if (Array.isArray(images)) {
            product.images = mapProductImages(images, product.name).map(image => AppDataSource.getRepository(ProductImage).create(image));
        }
        if (Array.isArray(variants)) {
            product.variants = mapProductVariants(variants).map(variant => AppDataSource.getRepository(ProductVariant).create(variant));
        }
        if (Array.isArray(designAreas)) {
            product.designAreas = mapDesignAreas(designAreas).map(area => AppDataSource.getRepository(DesignArea).create(area));
        }
        const saved = await productRepo.save(product);
...
export const createDesignerAccount = async (req, res, next) => {
    try {
        ...
        sendVerificationEmail(saved, emailVerificationToken).catch(err => console.error('Failed to send verification email to designer:', err));
        const { passwordHash: _pw, emailVerificationToken: _evt, emailVerificationExpiry: _eve, passwordResetTokenHash, passwordResetExpiry, ...safeUser } = saved;
